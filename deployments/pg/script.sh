...
function single(){
    ...
    case $cmd in
        "start")
            docker compose -f ./deployments/pg/single/docker-compose.yaml up
            ;;
        "stop")
            docker compose -f ./deployments/pg/single/docker-compose.yaml down
            ;;
        "clean")
            docker compose -f ./deployments/pg/single/docker-compose.yaml down
            if [ -d ./deployments/pg/single/dbfiles ]; then
                rm -rf ./deployments/pg/single/dbfiles
            fi
            ;;
        *)
